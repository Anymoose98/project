...
<template>
    ...
    <div class="arena-content">
        ...
        <div class="mech-card">
          <div class="mech-plate">
            ...
            <div class="mech-stats">
              ...
              <p class="stat-line">Stile Combattimento: <span class="stat-value">{{ robot.stileDiCombattimento }}</span></p>
              <p class="stat-line">Punteggio: <span class="stat-value">{{ robot.punteggioRobot }}</span></p>
              <p class="stat-line">Team: <span class="stat-value">{{ robot.teamRobotica ? robot.teamRobotica.nome : 'N/A' }}</span></p>
              <p class="stat-line">Tornei: <span class="stat-value">{{ robot.tornei ? robot.tornei.map(t => t.nome).join(', ') : 'Nessuno' }}</span></p>
            </div>
          </div>
        </div>
      </div>
      ...
      <router-link to="/" class="back-button">Torna alla Galleria</router-link>
    </div>
  </div>
    </main>
</template>
...
@keyframes pulse {
  0% { box-shadow: 0 0 12px rgba(255, 0, 122, 0.7); }
  50% { box-shadow: 0 0 20px rgba(255, 0, 122, 1); }
  100% { box-shadow: 0 0 12px rgba(255, 0, 122, 0.7); }
}
